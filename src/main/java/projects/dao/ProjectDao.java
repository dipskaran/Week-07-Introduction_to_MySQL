...
public class ProjectDao extends DaoBase {
	...
	public List<Project> fetchAllProjects() {
		// @formatter:off
		String sql = ""
			+ "SELECT * FROM "+PROJECT_TABLE+" ORDER BY project_name desc";
		// @formatter:on
		try(Connection conn =DbConnection.getConnection()) {
			startTransaction(conn);
			try(PreparedStatement stmt =conn.prepareStatement(sql)) {
				...
				try(ResultSet rs=stmt.executeQuery()) {
					...
					while(rs.next()) {
						...
						project.setEstimatedHours(rs.getBigDecimal("actual_hours"));
					...
					throw new DbException("ResultSet"+e);
				}
				
				
			} catch (Exception e) {
				rollbackTransaction(conn);
				throw new DbException("PreparedStatment"+e);
			}
			
		} catch (Exception e) {
			
			throw new DbException("Connection"+e);
		}
	}
		...
		try(Connection conn = DbConnection.getConnection()) {
			...
			try {
				...
				if(Objects.nonNull(project)) {
					...
					project.getCategories().addAll(fecthCategoriesForProject(conn,projectId));
	...
	private List<Category> fecthCategoriesForProject(Connection conn,
			Integer projectId) throws SQLException{
		// @formatter:off
		String sql = ""
